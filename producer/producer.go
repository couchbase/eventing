...
func (p *Producer) handleV8Consumer(vbnos []uint16, index int) {
	...
	c := consumer.NewConsumer(p.dcpStreamBoundary, p.cleanupTimers, p.eventingAdminPort, p.eventingDir, p, p.app,
		vbnos, p.bucket, p.logLevel, p.tcpPort, p.uuid, p.socketWriteBatchSize, p.timerWorkerPoolSize, index)

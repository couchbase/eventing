...
func TestCodeInsight(t *testing.T) {
	...
	if itemCount != eventCount {
		failAndCollectLogs(t, "For", functionName, "expected", itemCount, "got", eventCount)
	}
	...
	if err != nil {
		failAndCollectLogs(t, "For", functionName, "got response", string(response), "which doesn't parse", err)
	}

	insight := result["TestCodeInsight"]
	if !strings.Contains(insight.Script, `'use strict'`) {
		failAndCollectLogs(t, "For", functionName, "got response", string(response), "script not okay")
	}
	...
	if !end_ok {
		failAndCollectLogs(t, "For", functionName, "got insight", string(response),

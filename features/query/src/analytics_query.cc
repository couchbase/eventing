...
::Info Query::AnalyticsController::do_build(void *cookie) {
  lcb_cmdanalytics_create(&cmd_);
  auto result = lcb_cmdanalytics_statement(cmd_, query_info_.query.c_str(),
                                           query_info_.query.size());
  if (result != LCB_SUCCESS) {
    return ErrorFormat("Unable to set analytics query", connection_, result);
  }
  ...
  for (const auto &[key, value] : query_info_.named_params) {
    ...
    std::string key4 = "$" + key3;
    result = lcb_cmdanalytics_named_param(cmd_, key4.c_str(), key4.size(),
  ...
  for (const auto &param : query_info_.pos_params) {
    result =
        lcb_cmdanalytics_positional_param(cmd_, param.c_str(), param.size());
    if (result != LCB_SUCCESS) {
  ...
  result = lcb_cmdanalytics_consistency(
      cmd_, query_info_.options.analytics_consistency);
  if (result != LCB_SUCCESS) {
  ...
  if (query_info_.options.client_context_id != nullptr) {
    ...
    result = lcb_cmdanalytics_client_context_id(cmd_, id.c_str(), id.size());
    if (result != LCB_SUCCESS) {
  ...
  lcb_cmdanalytics_handle(cmd_, &handle_);
  lcb_cmdanalytics_callback(cmd_, RowCallback);

  auto timeout = UnwrapData(isolate_)->analytics_timeout;
  result = lcb_cmdanalytics_timeout(cmd_, timeout);
  if (result != LCB_SUCCESS) {
    return ErrorFormat("Unable to set timeout for query", connection_, result);
  }

  if (on_behalf_of_.size() != 0) {
    lcb_cmdanalytics_on_behalf_of(cmd_, on_behalf_of_.c_str(),
                                  on_behalf_of_.size());
  }
...
lcb_STATUS Query::AnalyticsController::run() {
  auto result = lcb_analytics(connection_, nullptr, GetCmd());
  if (result == LCB_SUCCESS) {
...
void Query::AnalyticsController::cancel() {
  lcb_analytics_cancel(connection_, GetHandle());
}

::Info Query::AnalyticsController::ErrorFormat(const std::string &message,
...
bool Query::AnalyticsController::IsStatusSuccess(const std::string &row) {
  std::regex re_status_success(R"("status"\s*:\s*"success")");
  return std::regex_search(row, re_status_success);
}

void Query::AnalyticsController::RowCallback(lcb_INSTANCE *connection, int,
  ...
  cursor->is_last = lcb_respanalytics_is_final(resp);
  cursor->client_err_code = lcb_respanalytics_status(resp);
...
::Info Query::AnalyticsController::ValidateQuery(
...
Query::Info Query::AnalyticsController::GetQueryInfo(
  ...
  if (args[1]->IsArray()) {
    if (auto info = helper->GetAnalyticsPosParams(args[1]); info.is_fatal) {
      return {true, info.msg};
    } else {
      std::swap(query_info.pos_params, info.pos_params);
    }
  } else if (args[1]->IsObject()) {
    if (auto info = helper->GetAnalyticsNamedParams(args[1]); info.is_fatal) {
      return {true, info.msg};
    } else {
  ...
  if (auto info =
          helper->opt_extractor_.ExtractAnalyticsOptions(args[2], options);
      info.is_fatal) {
    return {true, info.msg};
  }
...
void Query::AnalyticsFunction(const v8::FunctionCallbackInfo<v8::Value> &args) {

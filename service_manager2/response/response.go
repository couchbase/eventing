...
func GetErrCodes() ErrorPayloadList {
	errList := make([]ErrorInfo, 0, len(errMap))
	for _, errors := range errMap {
		errList = append(errList, errors)
	}

	epl := ErrorPayloadList{
		HeaderKey: headerKey,
		Version:   version,
		Revision:  revision,
		Errors:    errList,
...
func (r *responseWriter) systemLog(runtimeInfo *RuntimeInfo) {
	...
	common.LogSystemEvent(systemEventId, severity, extraAttributes)
}
